...
# args = [ 'line 1', 'line 2', ... ]
def proc_input(args):
	return([ stoi(i) for i in args[1:] ])

def solve(args, verbose=False):
	rooms = proc_input(args)
	count = reduce(lambda acc, (x, y): acc + (x + 2 <= y), rooms, 0)
	if verbose:
		print count
	return count

def test():
	assert(solve([ '3', '1 1', '2 2', '3 3' ]) == 0)
	assert(solve([ '3', '1 10', '0 10', '10 10' ]) == 2)
