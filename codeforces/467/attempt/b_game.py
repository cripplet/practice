...
def proc_input(args):
	(n, m, k) = stoi(args[0])
	armies = [ int(x) for x in args[1:] ]
	return(n, m, k, armies)

def itob(i):
	o = [ 0 ] * 32
	b = [ int(x) for x in list(bin(i)[2:]) ]
	o[-len(b):] = b
	return o

def solve(args, verbose=False):
	(n, m, k, armies) = proc_input(args)
	f = itob(armies[-1])
	count = 0
	for a in armies[:-1]:
		ap = itob(a)
		count += (sum([ f[i] ^ ap[i] for i in xrange(32) ]) <= k)
	if verbose:
		print count
	return count

def test():
	assert(solve([ '7 3 1', '8', '5', '111', '17' ]) == 0)
	assert(solve([ '3 3 3', '1', '2', '3', '4' ], verbose=True) == 3)
