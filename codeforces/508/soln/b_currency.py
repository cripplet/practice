...
def solve(args, verbose=False):
	n = list(proc_input(args))
	l = n[-1]
	kf = kl = -1
	succ = False
	for kp, d in enumerate(n):
		if int(d) % 2 == 0:
			if d < l and kf == -1:
				kf = kp
			if d > l:
...
def test():
	assert(solve([ '527' ], verbose=True) == 572)
	assert(solve([ '4573' ], verbose=True) == 3574)
	assert(solve([ '1357997531' ], verbose=True) == -1)
	assert(solve([ '443' ], verbose=True) == 434)
	assert(solve([ '227' ], verbose=True) == 722)
