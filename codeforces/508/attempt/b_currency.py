...
def solve(args, verbose=False):
	n = list(str(proc_input(args)))
	l = n[-1]
	# sorted
	e = [ (k, v) for (k, v) in enumerate(n) if not int(v) % 2 ]
	f = [ (k, v) for (k, v) in e if v < l ]
	res = -1
	if e:
		s = e[-1][0]
		if f:
...
def test():
	assert(solve([ '527' ]) == 572)
	assert(solve([ '4573' ]) == 3574)
	assert(solve([ '135' ]) == -1)
	assert(solve([ '243' ]) == 342)
	assert(solve([ '6245' ]) == 6542)
	assert(solve([ '6241' ]) == 6214)
