...
def solve(args, verbose=False):
	tasks = list(enumerate(proc_input(args)))
	tasks = [ (k + 1, v) for (k, v) in tasks ]
	tasks.sort(key=lambda(k, v): v)
	...
	for v in count:
		if len(count[v]) >= 2:
			perm.append(v)
		if len(count[v]) > 2 or len(perm) == 2:
			succ = True
			break
	output = [ [], [], [] ]
	if succ:
		...
		for v in sorted(count.keys()):
			if v not in perm:
				...
				tmp = collections.deque(count[v])
				for i in xrange(3):
					output[i] += tmp
					if not curr or (curr and i):
						tmp.rotate()
					# 2-element list -- move onto next perm
					if list(tmp) == count[v]:
						curr += 1
	if verbose:
		print btos(succ)
...
def test():
	solve([ '4', '1 3 3 1' ], verbose=True)
	solve([ '5', '2 4 1 4 8' ], verbose=True)
